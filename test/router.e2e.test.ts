...
test("routes to cheapest model when provider.sort.by is set to price", async () => {
  const customConfig = {
    ...config,
    provider: {
      ...config.provider,
...
test("routes to high throuput model when provider.sort.by is set to throughput", async () => {
  const customConfig = {
    ...config,
    provider: {
      ...config.provider,
  ...
  const response = await app.inject({
    method: "POST",
    url: "/chat",
    body: { question: "What is a Distributed System in Computer Science?" },
  });
  assert.equal(response.statusCode, 200);
  const body = response.json() as LLMResponse;
